<?php
/*
  $Id: general.php,v 1.160 2006/11/26 08:32:47 hpdl Exp $
...
function tep_admin_check_login() {
  ...
  if (!tep_session_is_registered('login_id')) {
    ...
    if ($filename != FILENAME_DEFAULT && $filename != FILENAME_FORBIDDEN && $filename != FILENAME_LOGOFF_ADMIN && $filename != FILENAME_ADMIN_ACCOUNT && $filename != FILENAME_POPUP_IMAGE && $filename != 'packingslip.php' && $filename != 'invoice.php' && $filename != 'address.php') {
  ...
  function tep_get_all_get_params($exclude_array = '') {
    ...
    return $get_url;
      ...
      function tep_date_long($raw_date) {
        ...
        return tep_strftime(strftime(DATE_FORMAT_LONG, mktime($hour, $minute, $second, $month, $day, $year)));
      }
...
// EOF: KategorienAdmin / OLISWISS
 ...
 function tep_browser_detect($component) {
  ...
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    $zip = $postcode;
    $telephone = tep_output_string_protected($address['telephone2']);
...
function tep_address_for_post($delivery_address, $eol) {
/*
 * Дубликат предыдущей функции tep_address_format
 * Но без обращения к таблице форматов адреса
 * Используются переменные из этой таблицы, которых достаточно для адреса посылки
 * $firstname $lastname$cr$streets$cr$city, $postcode$cr$statecomma$country
 */

      if (isset($delivery_address['firstname']) && tep_not_null($delivery_address['firstname'])) {
      $firstname = tep_output_string_protected($delivery_address['firstname']);
      $lastname = tep_output_string_protected($delivery_address['lastname']);
    } elseif (isset($delivery_address['name']) && tep_not_null($delivery_address['name'])) {
      $firstname = tep_output_string_protected($delivery_address['name']);
      $lastname = '';
    } else {
      $firstname = '';
      $lastname = '';
    }
      $street = tep_output_string_protected($delivery_address['street_address']);
      $city = tep_output_string_protected($delivery_address['city']);
      $state = tep_output_string_protected($delivery_address['state']);

      if (isset($delivery_address['country_id']) && tep_not_null($delivery_address['country_id'])) {
      $country = tep_get_country_name($delivery_address['country_id']);

      if (isset($delivery_address['zone_id']) && tep_not_null($delivery_address['zone_id'])) {
        $state = tep_get_zone_code($delivery_address['country_id'], $delivery_address['zone_id'], $state);
      }
    } elseif (isset($delivery_address['country']) && tep_not_null($delivery_address['country'])) {
      $country = tep_output_string_protected($delivery_address['country']);
    } else {
      $country = '';
    }
    ...
    $statecomma = '';
    $streets = $street;
    if ($country == '') $country = tep_output_string_protected($delivery_address['country']);
    if ($state != '') $statecomma = $state . ', ';

      $postcode = tep_output_string_protected($delivery_address['postcode']);
      $telephone = tep_output_string_protected($delivery_address['telephone2']);

      $delivery_address = $firstname . " " . $lastname . $eol;
      $delivery_address .= $telephone . $eol;
      $delivery_address .= $streets . $eol;
      $delivery_address .= $city . ", " . $postcode . $eol;
      $delivery_address .= $statecomma . $country;
...
// Sets the status of a product review
  function tep_set_review_status($review_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_REVIEWS . " set reviews_status = '1' where reviews_id = '" . (int)$review_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_REVIEWS . " set reviews_status = '0' where reviews_id = '" . (int)$review_id . "'");
    } else {
      return -1;
    }
  }
